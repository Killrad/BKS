...
function BlockCipher(params){
    ...
    for(let i = 0; i < count_bloks; i++){
        T_new.push(encoding_BC(T[i], params, replacementMatrix)); //функция возвращает массив символов
        
        for(let k = 0; k < T_new[i].length; k++){
            let buf = T_new[i][k].charCodeAt(0);
            encode_bin.push(buf.toString(2).padStart(16, '0'));
        }
    }
    
    return {message: mess, binary_message: binary_message, encode: T_new, encode_binary: encode_bin};
}
function encoding_BC(t, params, table){
    let T = [];
    let T_rez;
    let arr_T = [];
    let rezult_str = [];
    ...
    for(let i = 0; i < arr_T.length; i++){
        rezult_str.push(description_nonGraphic_symbols(parseInt(Math.abs(arr_T[i]), 2)));
    }
